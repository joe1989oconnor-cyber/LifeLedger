...
// Fetches transactions for a given account and identifies recurring bills
// All TrueLayer API calls happen server-side — token never exposed to browser
...
// Transaction types that suggest recurring bills
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('TrueLayer transactions error:', response.status, body);
    ...
    const allTxns = transactions.map(txn => {
      ...
      const type = txn.transaction_type || txn.transaction_classification?.[0] || '';
      const isRecurring = RECURRING_TYPES.includes(type) || txn.transaction_category === 'BILL_PAYMENT';

      // Try to match a known bill provider
      let billMatch = null;
      if (isOutgoing) {
    ...
    // ── Detect recurring bills by frequency analysis ───────────────────────
    // For unmatched DDs/SOs, check if the same description appears monthly
    const unmatchedRecurring = allTxns.filter(t => t.recurring && !t.billMatch && t.amount < 0);
    const descGroups = {};
    unmatchedRecurring.forEach(t => {
      const key = t.desc.toLowerCase().replace(/\s+/g, ' ').trim();
      if (!descGroups[key]) descGroups[key] = [];
      descGroups[key].push(t);
    });

    // Mark multi-occurrence unmatched transactions as potential bills
    Object.entries(descGroups).forEach(([desc, txns]) => {
      if (txns.length >= 2) {
        const avgAmt = txns.reduce((a, t) => a + Math.abs(t.amount), 0) / txns.length;
        txns.forEach(t => {
          if (!t.billMatch) {
            t.billMatch = {
              prov: toTitleCase(t.desc),
              cat: 'Other',
              amt: parseFloat(avgAmt.toFixed(2)),
              freq: 'Monthly',
              emoji: '📋'
            };
          }
    ...
    // Deduplicate bill suggestions (keep most recent per provider)
    const billsMap = {};
    allTxns.forEach(t => {
      if (t.billMatch) {
        const key = t.billMatch.prov.toLowerCase();
        if (!billsMap[key] || t.date > billsMap[key].date) {
          billsMap[key] = t;
        }
      }
    });

    const suggestedBills = Object.values(billsMap).sort((a, b) =>
      (a.billMatch.cat || '').localeCompare(b.billMatch.cat || '')
    );

    return res.status(200).json({
      transactions: allTxns.slice(0, 50), // Return last 50 for display
      suggestedBills,
